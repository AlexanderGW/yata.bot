...
describe('Backtest dataset 1', () => {
            ...
            for (let i = 0; i < subscribe.timeframeAny.length; i++) {
                ...
                for (let j = 0; j < timeframe.result.length; j++) {
                    ...
                    if (result?.length) {
                        ...
                        let strategy = await Bot.getItem(uuid) as StrategyItem;
    ...
    const botTimeframeHandler = async function (
        ...
        Bot.subscribe({
            ...
            action: await botSubscriptionActionCallback,
    ...
    before(async function () {
        ...
        try {
            ...
            // Load from storage
            let response: any = await fs.readFileSync(
        ...
        stratBullishRsi14Oversold = await Strategy.new({
            action: [
                [await scenarioBullishRsi14Oversold],
            ],
            analysis: [
                await  analysisRsi14,
            ],
        ...
        stratBullishMacd12_26_9Crossover = await Strategy.new({
            action: [
                [await scenarioBullishMacd12_26_9Crossover],
            ],
            analysis: [
        ...
        stratBullishSma20Cross = await Strategy.new({
            action: [
                [await scenarioSma20CrossUp],
            ],
            analysis: [
                await analysisSma20,
            ],
        ...
        stratBullishBollinger20LowerCross = await Strategy.new({
            action: [
                [await scenarioBollingerBullishLowerCrossover],
            ],
            analysis: [
    ...
    it('should match known "stratBullishRsi14Oversold" scenarios', async () => {
        ...
        let defaultTimeframe = await Timeframe.new({
